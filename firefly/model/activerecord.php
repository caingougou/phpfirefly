...
class Model {
	...
	public function __construct($model_name) {
		...
		$this->dbh = call_user_func(
			array(ucfirst($config['db'][ENVIRONMENT]['adapter']), 'establish_connection'), 
			$config['db'][ENVIRONMENT]['host'], 
			$config['db'][ENVIRONMENT]['username'], 
			$config['db'][ENVIRONMENT]['password'], 
			$config['db'][ENVIRONMENT]['database']);
	...
	public function find() {
		...
		if(($args[0] == 'first') || ($args[0] == 'all')) {
			return call_user_func(array($this->model_name, '_find_record'), $this, $args[0], $args[1]);
		} elseif(is_int($args[0]) || is_array($args[0])) {
			return call_user_func(array($this->model_name, '_find_record_by_primary_key'), $this, $args[0]);
		} else {
	...
	public function count($params = null) {
		...
		$counter = $ar->{$this->table_name.'_count'};
	...
	public function create($data = null) {
		$ar = call_user_func(array($this->model_name, 'create_record'), $this, $data);
...
*/
	...
	public function __construct() {
		if(!isset($this->model)) $this->model = self::$instances[get_class($this)];
		return $this;
	}

	public function save($data = null, $conditions = null) {
		if($this->model->dbh->execute($this->_insert_sql($data, $conditions))) {
			$this->{$this->model->primary_key} = $this->model->dbh->last_insert_id();
	...
	public function update($changes = null) {
		...
		$sql = $this->_add_conditions(substr($sql, 0, -2), array($this->model->table_name.'.'.$this->model->primary_key.' = ?',$this->{$this->model->primary_key}));
	...
	public function delete() {
		$sql = 'DELETE FROM `'.$this->model->table_name.'` WHERE `'.$this->model->primary_key.'` = '.$this->{$this->model->primary_key};
	...
	public function _find_record_by_primary_key($model, $ids) {
		...
		if(is_array($ids)) {
			$sql = self::_add_conditions($sql, array('`'.$model->primary_key.'` IN (?)', $ids));
			$ars = array();
			$rs = $model->dbh->execute($sql);
			while(false !== ($row = $model->dbh->fetch($rs))) {
				$ar = self::create_record($model, $row);
			...
			if($ars == array() ) return false;
			return $ars;
		} else {
			$sql = self::_add_conditions($sql, array('`'.$model->primary_key.'` = ?', $ids));
	...
	public function __get($key) {
		if($key == 'dbh' || $key == 'model_name' || $key == 'table_name' || $key == 'primary_key' || $key == 'table_columns' || $key == 'table_columns_types' || $key == 'table_columns_sizes' || $key == 'has_one' || $key == 'has_many' || $key == 'belongs_to'|| $key == 'has_many_and_belongs_to') return $this->$key;
		if(in_array($key, $this->model->table_columns)) {
			return $this->$key;
		} elseif ($this->model->has_one != null && in_array($key, $this->model->has_one)) {
			$ref_model_name = ucfirst($key);
			$foreign_key = $ref_model.'_id';
			return $ref_model->find('first', array('conditions' => array($this->model->primary_key.' = ?', $this->model->{$foreign_key})));
		} elseif ($this->model->has_many != null && in_array($key, $this->model->has_many)) {
			$ref_model_name = ucfirst(Inflector::singular($key));
			$ref_model = call_user_func(array($ref_model_name, 'model'));
			return $ref_model->find('all', array('conditions' => array('`'.strtolower($this->model->model_name).'_id` = ?', $this->{$this->model->primary_key})));
			...
			$ref_model_name = ucfirst($key);
//			$ref_model = new $ref_model_name;
			return self::find_record('first', array('conditions' => array('`'.$this->table_name.'`.`'.$this->primary_key.'` = ?', $this->{$this->primary_key}), 'join' => $reference_model));
			...
			$hmbt = new $hmbt_name;
			return $ref_model->find('all', array('conditions' => array($this->table_name.'.'.$this->primary_key.' = ?', $this->{$this->primary_key}), 'join' => $hmbt->table_name.' on '.$hmbt->table_name.'.'.$reference_model->primary_key.' = '.$reference_model->table_name.'.'.$reference_model->primary_key.' join '.$this->table_name.' on '.$hmbt->table_name.'.'.$this->primary_key.'='.$this->table_name.'.'.$this->primary_key,'group' => 'group by '.$reference_model->table_name.'.'.$reference_model->primary_key));
		} else {
